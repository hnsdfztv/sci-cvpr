...
parser.add_argument('--device', type=str, default='cpu',
                    help='device for finetune: cpu or cuda')
parser.add_argument('--lr', type=float, default=0.0005, help='learning rate')
parser.add_argument('--seed', type=int, default=2, help='random seed')
parser.add_argument('--save', type=str, default='results/finetune/',
                    help='location of the data corpus')
parser.add_argument('--model', type=str, default='./weights/difficult.pt',
                    help='location of the data corpus')
parser.add_argument('--train_data_path', type=str, default='./data/finetune',
                    help='path to finetune train images')
parser.add_argument('--test_data_path', type=str, default='./data/finetune',
                    help='path to finetune test images')
parser.add_argument('--save_model', type=str, default='./weights/finetune_latest.pt',
                    help='path to save finetuned model weights')
            ...
            if total_step % 10 == 0 and total_step != 0:
                ...
                with torch.no_grad():
                    for _, (input, image_name) in enumerate(test_queue):
                        ...
                        u_name = '%s.png' % (
                            image_name + '_' + str(total_step) + '_ref_')
                        u_path = args.save + '/' + u_name
                        save_images(ref, u_path)

    torch.save(model.state_dict(), args.save_model)
    print('finetuned model saved to {}'.format(args.save_model))
